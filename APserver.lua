...
--    print("Vars   : " .. vars  .. "\n\n\n")
  ...
  conn:on("sent",function(conn) 
    if responseBytes>=0 and method=="GET" then
        if file.open("setup.html", "r") then            
...
function parse_wifi_credentials(vars)
    ...
    print("Password is encrypted with AES.")
    
    wifi_password_enc = crypto.encrypt("AES-ECB", "1[($72;#WsMP4Hs/", wifi_password)
    file.open("credentials.txt", "w+")
    file.writeline(wifi_ssid)
    file.writeline(wifi_password_enc)
    file.close()
    return true
end
